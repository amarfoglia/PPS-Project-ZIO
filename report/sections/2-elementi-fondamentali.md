...
```
Non è possibile sostituire il corpo di `echo` con il suo valore perché `val echo: Unit = ()` rappresenta un programma differente. Questo è dovuto alla presenza di _side effects_. I costruttori di ZIO, come `attempt`, permettono di ricondurre il codice soprastante a _funzioni pure_ (che soddisfano la _trasparenza referenziale_) catturando i _side effects_, cioè tracciando la loro descrizione, senza però eseguirli.
